...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create test-1-e6e7
  fly apps create test-1-e6e7-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app test-1-e6e7
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app test-1-e6e7-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'test-1-e6e7-staging' does not match app name in config file 'test-1-e6e7'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name test-1-e6e7-db
  fly postgres attach --app test-1-e6e7 test-1-e6e7-db

  fly postgres create --name test-1-e6e7-staging-db
  fly postgres attach --app test-1-e6e7-staging test-1-e6e7-staging-db
  ```
